"""
RTC Scholar - Production RAG Agent (Optimized for Render)
==========================================================
Lightweight, fast, and accurate retrieval without heavy dependencies
"""
...
# ============================================
# ADVANCED VECTOR DB - PRODUCTION GRADE
# ============================================
class ProductionVectorDB:
    ...
    def __init__(self):
        self.documents = []
        self.doc_frequencies = {}  # For IDF calculation
    ...
    def calculate_entity_bonus(self, query: str, doc: str) -> float:
        ...
        # Named entities (people, places, programs)
        entities = [
    ...
    def search(self, query: str, top_k: int = 3) -> List[str]:
        ...
        if scored_docs:
            ...
            print(f"   Top match score: {scored_docs[0][0]:.2f}")
...
def call_llm(prompt: str, context: str) -> str:
    ...
    if not OPENROUTER_API_KEY:
        return "⚠️ Configuration needed. Please set OPENROUTER_API_KEY environment variable."
    
    system_prompt = f"""You are RTC Scholar, a helpful AI assistant for Rathinam Technical Campus.

CONTEXT (answer using ONLY this information):
{context}

RULES:
1. Answer directly from the context above
2. Be concise and accurate
3. Quote names/titles exactly as shown
4. If info is in context, answer confidently
5. Keep response under 150 words
6. End with: "Need anything else? 😊"
...
Personality: Friendly, professional, helpful"""
    ...
    try:
        ...
        return "Sorry, the response took too long. Please try again! ⏱️"
...
def root():
    ...
    return jsonify({
        ...
        'status': 'running',
        'version': '3.0',
        'features': ['BM25 ranking', 'Query expansion', 'Phrase matching', 'Entity recognition'],
...
def dialogflow_webhook():
    """Dialogflow webhook with production RAG"""
    ...
    try:
        ...
        if not query_text:
            return jsonify({
                'fulfillmentText': 'Sorry, I didn\'t receive your question.'
            })
        ...
        if not relevant_docs:
            return jsonify({
                'fulfillmentText': 'Could you rephrase that? I want to give you accurate info! 🤔'
            })
        ...
        return jsonify({
            ...
            'source': 'webhook-production'
        })
    
    except Exception as e:
        print(f"❌ Error: {str(e)}")
        return jsonify({
            'fulfillmentText': 'Oops! Something went wrong. Please try again! 🔄'
        }), 500
...
def test_endpoint():
    ...
    # Search with scores
    results = vector_db.search_with_scores(query, top_k=3)
    relevant_docs = [doc for _, doc in results]
...
if __name__ == '__main__':
    ...
    print(f"🚀 RTC Scholar Production starting on port {port}")
    print(f"📊 Documents loaded: {len(vector_db.documents)}")
    print(f"🔑 API configured: {bool(OPENROUTER_API_KEY)}")
